...
import type { DynamicMeshStatus } from '../mesh/coordinator';
    ...
    <Page
      ...
      title={
        sharing
          ? status.active
            ? 'Sharing, and helping.'
            : 'Sharing is on, but nothing is reporting.'
          : 'Not sharing.'
      }
      onBack={onBack}
    >
      <View style={styles.chips}>
        <Chip
          label={sharing && status.active ? 'live' : 'paused'}
          tone={sharing && status.active ? 'strong' : 'warn'}
        />
        ...
        <Section label="Why some of this is off">
          <Card tone="outline" style={{ gap: space.sm }}>
            {status.blocked_by.map((reason) => (
              <Body key={reason} tone="soft" style={styles.small}>
                {reason}
              </Body>
            ))}
          </Card>
      ...
      <Section label="Nearby phone mesh">
        <Card tone="outline" style={{ gap: space.md }}>
          <MetaRow label="Relay service" value={mesh.running ? 'running' : 'stopped'} emphasis />
          <MetaRow label="Phones discovered" value={`${mesh.discoveredCount}`} />
          <MetaRow label="Stable direct links" value={`${mesh.peerCount}`} />
          <MetaRow label="Internet uplink" value={mesh.online ? 'available' : 'not on this phone'} />
          {mesh.problem ? (
            <Body tone="soft" style={styles.small}>
              {mesh.problem}
            </Body>
...
function Choice({ label, note, onPress }: { label: string; note: string; onPress: () => void }) {
  ...
  return (
    ...
    >
      <Body color={palette.ink} style={{ fontWeight: '700' }}>
        {label}
      </Body>
      <Body tone="soft" style={styles.small}>
        {note}
