...
export function MapView({ geometry }: { geometry: VenueGeometry }) {
  ...
  const trackPoints = (geometry.track ?? []).map((point) => `${toX(point.x)},${toY(point.y)}`).join(' ');
      ...
      <Svg width="100%" height="100%" viewBox={`0 0 ${width} ${height}`}>
        {Object.values(edges).map((edge) => {
          return (
            <Polyline
              key={edge.id}
              points={edge.geometry.map((point) => `${toX(point.x)},${toY(point.y)}`).join(' ')}
              fill="none"
              stroke={EDGE}
              strokeWidth={0.8}
              strokeOpacity={0.6}
        ...
        {gates.map((zone) => (
          <Circle key={zone.id} cx={toX(zone.position.x)} cy={toY(zone.position.y)} r={3} fill={GATE} />
        ))}
      </Svg>
    </View>
  );
