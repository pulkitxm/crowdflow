...
import { validateCircuitGeometry, VenueGraph } from '@crowdflow/core';
import { readPack } from '@crowdflow/cli/ingest';
import type { CircuitSummary, VenueGeometry } from '@crowdflow/contracts/wire';

export interface LoadedCircuit { pack: CircuitPack; track: Position[]; graph: VenueGraph; operational: boolean }
...
export function loadCircuit(root: string, id: string): LoadedCircuit {
  ...
  const validation = validateCircuitGeometry(pack, track);
  if (validation.problems.length) throw new Error(`${id} failed circuit validation: ${validation.problems.join('; ')}`);
  return { pack, track, graph: new VenueGraph(pack), operational: pack.capability === 'venue_reviewed' };
}
...
export function summary(circuit: LoadedCircuit): CircuitSummary {
  return { id: circuit.pack.id, name: circuit.pack.name, layout_id: circuit.pack.layout_id, capability: circuit.pack.capability, zones: Object.keys(circuit.pack.zones ?? {}).length, edges: Object.keys(circuit.pack.edges ?? {}).length, crossings: Object.keys(circuit.pack.crossings ?? {}).length, track_length_m: circuit.pack.track_length_m, untrustworthy_widths: Object.values(circuit.pack.edges ?? {}).filter((edge) => !isTrustworthy(edge.width_m)).length };
}
/**
