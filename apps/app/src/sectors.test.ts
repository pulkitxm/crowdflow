...
import type { LiveSnapshot, PeopleQueryResult, VenueGeometry, ZoneState } from '@crowdflow/contracts/wire';
...
function state(id: string, density: number, nodes: number): ZoneState {
  return {
    ...
    estimated_population: nodes,
    band: density >= 2 ? 'critical' : 'nominal',
    over_capacity: density >= 2,
    los_grade: density >= 2 ? 'F' : 'A',
    net_flow_per_min: 5,
  };
}
...
function live(): LiveSnapshot {
  return {
    ...
    state: {
      circuit_id: 'toy',
      timestamp: 10,
      zones: { west: state('west', 0.2, 5), west_walk: state('west_walk', 2.1, 3), east: state('east', 0.1, 4) },
      unobserved_zones: ['east_walk'],
    },
    reporting_devices: 12,
    accepted_total: 12,
    rejected_total: 0,
...
describe('live sectors', () => {
  it('groups venue zones and exact live crowd around named viewing sectors', () => {
    ...
    expect(rows.find((row) => row.id === 'west')).toMatchObject({
      zoneCount: 2,
      observedZoneCount: 2,
      people: 120,
      density: 2.1,
    ...
    expect(rows.find((row) => row.id === 'east')).toMatchObject({
      zoneCount: 2,
      observedZoneCount: 1,
      people: 35,
      density: 0.1,
