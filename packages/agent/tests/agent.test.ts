...
const pack: CircuitPack = {
  ...
  zones: {
    ...
    forbidden: { id: 'forbidden', kind: 'concourse', position: { x: 50, y: 0 } },
  ...
  crossings: {},
  constraints: { never_route_through: ['forbidden'], emergency_exits: ['b'] },
};
const state: VenueState = {
  ...
  session_id: null,
  zones: {},
  unobserved_zones: ['a', 'forbidden', 'b'],
};
...
describe('TypeScript Crowd Ops Agent', () => {
  it('cannot produce an unreviewed or dispatched proposal', async () => {
    ...
    const toolbox = new Toolbox({ pack, graph, safety: new SafetyEngine(pack), state, now: 0 });
  ...
  it('detects a gate departing from its own baseline without model arithmetic', () => {
    const engine = new InsightEngine(pack);
    const zone = (density: number) => ({
      zone_id: 'a',
      timestamp: 0,
      observed_nodes: 20,
  ...
  it('offers the complete read/propose surface and no dispatch tool', () => {
    ...
    const names = toolbox.schemas().map((tool) => tool.name);
    expect(names).toContain('get_event_schedule');
    expect(names).toContain('generate_insight');
