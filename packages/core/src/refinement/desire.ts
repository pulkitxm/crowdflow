...
export function proposeDesireLines(fragments: TraceFragment[], pack: CircuitPack, detourMin = ASSUMED_DESIRE_LINE_DETOUR_MIN, minimum = ASSUMED_DESIRE_LINE_MIN_FRAGMENTS): DesireLine[] {
  ...
  return [...groups.entries()].filter(([, group]) => group.ids.length >= minimum).map(([id, group]) => {
    ...
    return { id: `desire-${source}-${destination}`, from_zone: source!, to_zone: destination!, start: group.start, end: group.end, observed_length_m: observed, graph_walk_m: graphWalk, detour_ratio: graphWalk / observed, fragments: [...group.ids].sort(), evidence: group.ids.length, width_m: lateralWidth(fragments.filter((fragment) => group.ids.includes(fragment.fragment_id)), group.start, group.end) };
