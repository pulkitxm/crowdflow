...
export class Simulation {
  ...
  get active(): number {
    return this.agents.filter((agent) => agent.started && !agent.arrived).length;
  }
  get arrived(): number {
    return this.agents.filter((agent) => agent.arrived).length;
  }
  get stranded(): number {
    return this.agents.filter((agent) => agent.stranded).length;
  }
}
