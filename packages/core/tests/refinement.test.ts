...
describe('private participation and refinement', () => {
  ...
  it('returns unknown when capture channels do not overlap', () => {
    expect(estimateParticipation(sketch([...Array(100).keys()]), sketch([...Array(100).keys()].map((n) => n + 10000)), 10)).toBeNull();
  });
  ...
  it('writes trustworthy measured capacity back without creating geometry', () => {
    const pack: CircuitPack = {
      id: 'line', name: 'Line', geometry_source: 'synthetic', track_length_m: 100, altitude_m: 0,
      frame: { origin_lat: 0, origin_lon: 0, track_bounds_m: [100, 1], venue_bounds_m: [0, 0, 100, 1] },
      zones: { a: { id: 'a', kind: 'gate', position: { x: 0, y: 0 } }, b: { id: 'b', kind: 'exit', position: { x: 100, y: 0 } } },
      edges: { ab: { id: 'ab', source: 'a', destination: 'b', length_m: 100, width_m: { value: 2, provenance: 'assumed' } } }, crossings: {}, constraints: {},
    };
    const fragments: TraceFragment[] = Array.from({ length: 40 }, (_, index) => ({
      fragment_id: `f${index}`, points: [{ x: 0, y: index % 2 ? 1 : -1 }, { x: 100, y: index % 2 ? 1 : -1 }],
      t_start: index * 60, t_end: index * 60 + 50, epsilon: 10, noise_radius_m: 0.1,
    }));
