...
const pack: CircuitPack = {
  ...
  edges: { ab: { id: 'ab', source: 'a', destination: 'b', length_m: 100, width_m: sourced } },
...
describe('seeded TypeScript simulation', () => {
  ...
  it('is reproducible and emits the shared CrowdNode shape', () => {
    const run = () => {
      ...
      const scenario = new Scenario('walk', 'walk', [{ count: 20, origin: 'a', destination: 'b', start_s: 0, spread_s: 0 }], 100, 42);
